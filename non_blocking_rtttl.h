...
class Player {
      ...
      this->continuousPlay = false;
		...
		bool pollSong()
		{
			...
			if(stepDue()){
				...
				if(!nextStep()){
					if (this->continuousPlay) initSong(); //restart song only if continuousPlay is true
    ...
    /** Change continuousPlay setting for pollSong(): 
		 * false(default): when song finish stop play
		 * true: when song finished restart it*/
		void setContinuousPlay(bool continuous){
